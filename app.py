...
with st.sidebar:
    ...
    st.divider()
    st.caption("📌 v3.0 | Optimizovana verzija")
...
def parsiraj_klase_i_atribute(puml_kod):
    """Parsira klase i atribute iz PlantUML koda."""
    klase_atributi = []
    trenutna_klasa = None
    unutar_klase = False
    nasljedjivanja = {}
    ...
    for linija in puml_kod.splitlines():
        ...
        if naslj_match:
            nasljedjivanja[naslj_match.group(2)] = naslj_match.group(1)

        naslj_match2 = re.match(r'^\s*(\w+)\s*--\|>\s*(\w+)', linija)
        if naslj_match2:
            nasljedjivanja[naslj_match2.group(1)] = naslj_match2.group(2)
    ...
    for linija in puml_kod.splitlines():
        ...
        if match:
            ...
            trenutna_klasa = {
                "naziv": match.group(2),
                "roditelj": nasljedjivanja.get(match.group(2)),
                "atributi": []
            }
            unutar_klase = True
            continue
...
def parsiraj_relacije(puml_kod):
    ...
    for linija in puml_kod.splitlines():
        ...
        elif "--" in s and "<|--" not in s and "--|>" not in s and s != "--":
...
def generisi_pdf(opis, puml_kod, sql_kod, evaluacija, putanja_png):
    ...
    for kljuc, vrijednost in evaluacija.items():
        if kljuc not in ['provider', 'has_startuml', 'has_enduml', 'rendered_ok']:
...
if st.button("✨ Generiši dijagram", type="primary"):
    ...
    try:
        ...
        raw_puml = generate_puml(opis_sistema)
        progress_bar.progress(40)

        status_tekst.text("🔧 Sanitizacija PlantUML koda...")
        puml = sanitize_puml(raw_puml)
        ...
        if code != 0:
            ...
            with st.expander("🔍 Sirovi PlantUML kod"):
                st.code(raw_puml, language="text")
            with st.expander("🔧 Sanitizovani PlantUML kod"):
        ...
        evaluacija = evaluate_puml(puml, putanja_png.exists())
        ...
        with kolona2:
            ...
            <table style="width:100%; font-size:13px;">
                ...
                <tr><td>🧬 Generalizacija</td><td><b>{broj_nasljedjivanja}</b></td><td>{semafor(broj_nasljedjivanja,3,1)}</td></tr>
            </table>
            """, unsafe_allow_html=True)

            st.markdown("### 🔗 Multiplikativnost")
            st.markdown(f"""
            <table style="width:100%; font-size:13px;">
                <tr style="background-color:#f0f0f0;"><th>Oznaka</th><th>Naziv</th><th>Broj</th><th>Status</th></tr>
                <tr><td><b>1 : 1</b></td><td>One-to-One</td><td align="center"><b>{jedan_jedan}</b></td><td align="center">{'🟢' if jedan_jedan > 0 else '⚪'}</td></tr>
                <tr><td><b>1 : N</b></td><td>One-to-Many</td><td align="center"><b>{jedan_vise}</b></td><td align="center">{semafor(jedan_vise, 3, 1)}</td></tr>
                <tr><td><b>M : N</b></td><td>Many-to-Many</td><td align="center"><b>{vise_vise}</b></td><td align="center">{'🟢' if vise_vise > 0 else '⚪'}</td></tr>
            </table>
            """, unsafe_allow_html=True)
            ...
            with st.expander(f"📦 Detaljna analiza klasa ({broj_klasa} klasa)"):
                for klasa in klase_atributi:
                    lista_atributa = ", ".join(
                        [f"`{a['naziv']}` : {a['tip']}" for a in klasa["atributi"]]
                    )
                    st.markdown(f"**📦 {klasa['naziv']}**")
                    st.markdown(f"Atributi: {lista_atributa if lista_atributa else 'Nema dodatnih atributa'}")
                    if klasa.get("roditelj"):
                        st.markdown(f"Nasljeđuje od: `{klasa['roditelj']}`")
                    st.markdown("---")

            with st.expander(f"🔗 Detaljna analiza relacija ({broj_relacija})"):
                podaci_relacija = []
                for rel in relacije_lista:
                    if rel["tip"] == "Generalizacija":
                        match = re.match(r'(\w+)\s*<\|--\s*(\w+)', rel["tekst"])
                        if match:
                            podaci_relacija.append({
                                "Tip": "🧬",
                                "Entitet A": match.group(1),
                                "Entitet B": match.group(2),
                                "Naziv": f"{match.group(2)} je {match.group(1)}"
                            })
                    elif rel["tip"] == "Asocijacija":
                        match = re.match(r'(\w+)\s+"([^"]*)"\s+--\s+"([^"]*)"\s+(\w+)', rel["tekst"])
                        if match:
                            podaci_relacija.append({
                                "Tip": "🔗",
                                "Entitet A": match.group(1),
                                "Mult. A": f'"{match.group(2)}"',
                                "Entitet B": match.group(4),
                                "Mult. B": f'"{match.group(3)}"'
                            })
        ...
        with kol_kod1:
            ...
            st.code(puml, language="text")
            with st.expander("🔍 Sirovi LLM izlaz"):
                st.code(raw_puml, language="text")

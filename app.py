...
def parsiraj_klase_i_atribute(puml_kod):
    ...
    for linija in puml_kod.splitlines():
        ...
        if naslj_match:
            natklasa = naslj_match.group(1)
            potklasa = naslj_match.group(2)
            nasljedjivanja[potklasa] = natklasa

        naslj_match2 = re.match(r'^\s*(\w+)\s*--\|>\s*(\w+)', linija)
        if naslj_match2:
            potklasa = naslj_match2.group(1)
            natklasa = naslj_match2.group(2)
            nasljedjivanja[potklasa] = natklasa
    ...
    try:
        ...
        with kolona2:
            ...
            with st.expander(f"📦 Detaljna analiza klasa ({broj_klasa} klasa, {broj_atributa} atributa)"):
                st.markdown("**Prikaz: Naziv tabele, atributi i generalizacija**")
                st.markdown("---")
                ...
                for klasa in klase_atributi:
                    ...
                    with kol_a:
                        st.markdown(
                            f"**Atributi:** {lista_atributa if lista_atributa else 'Nema dodatnih atributa'}"
                        )

                    with kol_b:
                        if roditelj:
                            st.markdown(f"**Nasljeđuje od:** `{roditelj}`")
                            st.markdown(f"**Relacija:** `{roditelj} <|-- {naziv_klase}`")
                        else:
                    ...
                    if tip == "Generalizacija":
                        ...
                        if match:
                            ...
                            podaci_relacija.append({
                                ...
                                "Entitet B": potklasa,
                                "Mult. B": "—",
                                "Naziv asocijacije": f"{potklasa} je {natklasa}"
                            })
        ...
        with kol_s2:
            st.caption(f"⏱️ Vrijeme: {proteklo_vrijeme:.1f}s | 📌 v2.1 | Groq API")

...
with st.sidebar:
    ...
    with st.expander("🧬 Generalizacija"):
        st.write("Nasljeđivanje: `Dijete --|> Roditelj`")
...
def parsiraj_klase_i_atribute(puml_kod):
    ...
    for linija in puml_kod.splitlines():
        ...
        if naslj_match:
            nasljeđivanja[naslj_match.group(1)] = naslj_match.group(2)
        ...
        if unutar_klase and trenutna_klasa is not None:
            ...
            if attr_match:
                trenutna_klasa["atributi"].append({
                    "naziv": attr_match.group(2),
                    "tip": attr_match.group(3),
                    "je_pk": attr_match.group(1) is not None
                })
        ...
        with kolona2:
            ...
            with st.expander(f"📦 Detaljna analiza klasa ({broj_klasa} klasa, {broj_atributa} atributa)"):
                ...
                for klasa in klase_atributi:
                    ...
                    pk_attr = next((a for a in klasa['atributi'] if a['je_pk']), None)
                    pk_tekst = f"🔑 `{pk_attr['naziv']}` : {pk_attr['tip']}" if pk_attr else "⚠️ Nema PK"
                    ...
                    with kol_a:
                        st.markdown(f"**PK operacija:** {pk_tekst}")
                        st.markdown(f"**Atributi:** {lista_atributa}")
                    with kol_b:
                        if roditelj:
                    ...
                    if tip == "Generalizacija":
                        ...
                        if match:
                            ...
                            podaci_relacija.append({
                                ...
                                "Entitet B": roditelj,
                                "Mult. B": "—",
                                "Naziv asocijacije": f"Generalizacija ({dijete} je {roditelj})"
                            })
                ...
                if gen_relacije:
                    ...
                    for rel in gen_relacije:
                        ...
                        if match:
                            dijete = match.group(1)
                            roditelj = match.group(2)
                            st.markdown(f"- **A→B:** `{dijete} --|> {roditelj}`")
                            st.markdown(f"- **B→A:** `{roditelj} <|-- {dijete}`")
                            st.markdown(f"  *(čita se: {dijete} je {roditelj})*")
